...
const TEST_USER = {
  email: "ultra-plan@example.com",
  name: "Ultra Plan Test User",
  uid: "onboard-uid-ultra-plan",
};

/**
 * Build a mock user object for a given onboarding step with the "ultra" plan.
 */
function buildMockUser(step: number) {
  return {
    ...
    onboardingStep: step,
    plan: step >= 2 ? "ultra" : null,
    credits: step >= 50 ? 2500 : 0,
    maxCompanies: step >= 50 ? 100 : undefined,
    emailVerified: true,
  };
}
...
/**
 * Mock the /api/create-payment endpoint to return a fake client_secret for Ultra plan.
 */
async function mockCreatePayment(page: Page) {
  await page.route("**/api/create-payment**", async (route) => {
    await route.fulfill({
      ...
      contentType: "application/json",
      body: JSON.stringify({
        client_secret: "pi_mock_ultra_plan_secret_mock",
        type: "one_time",
        amount: 13900,
      }),
    });
...
test.describe("Onboarding - Ultra Plan - Step 1 (Plan Selection)", () => {
  test("step 1: clicking the 'Ultra Plan' button selects it", async ({
    page,
  }) => {
    ...
    const selectUltraBtn = page
      .getByRole("button", { name: "Select Plan" })
      .nth(2);
    await expect(selectUltraBtn).toBeVisible({ timeout: 10000 });
    await selectUltraBtn.click();
  ...
  test("step 1: selecting Ultra Plan and clicking submit advances to step 2", async ({
    page,
  }) => {
    ...
    // "Select Plan" buttons ordered Base(0), Pro(1), Ultra(2)
    const selectUltraBtn = page
      .getByRole("button", { name: "Select Plan" })
      .nth(2);
    await expect(selectUltraBtn).toBeVisible({ timeout: 10000 });
...
test.describe("Onboarding - Ultra Plan - Step 2 (Company Input)", () => {
  test("user with onboardingStep=2 (ultra plan) sees company input UI", async ({
    page,
  }) => {
  ...
  test("step 2: add company 'ultratech.com' and advance to step 3", async ({
    page,
  }) => {
    ...
    await companyInput.fill("linkedin.com/company/ultratech");
...
test.describe("Onboarding - Ultra Plan - Step 3 (Profile & CV)", () => {
  test("user with onboardingStep=3 (ultra plan) sees profile analysis UI", async ({
    page,
  }) => {
  ...
  test("step 3: upload PDF and advance to step 4 (advanced filters for ultra)", async ({
    page,
  }) => {
    ...
    await linkedinInput.fill("https://www.linkedin.com/in/ultraplantestuser");
    ...
    // Ultra plan: step 3 → step 4 (advanced filters)
    advanceTo(4);
...
test.describe("Onboarding - Ultra Plan - Step 4 (Advanced Recruiter Filters)", () => {
  test("user with onboardingStep=4 (ultra plan) sees advanced filters UI", async ({
    page,
  }) => {
  ...
  test("step 4: ultra plan shows up to 50 filter selection", async ({
    page,
  }) => {
    await mockUserAtStep(page, 4);
    await page.goto("/dashboard");
    ...
    // Ultra plan allows 50 filters — badge text: "{n} / 50 strategies"
    await expect(page.getByText(/\/ 50 strategies/i).first()).toBeVisible({
      timeout: 5000,
    });
  });

  test("step 4: clicking 'Continue Setup' without filters advances to step 5", async ({
    page,
  }) => {
    ...
    // The wrapper only has "Reset to Default" and "Continue Setup" buttons (no "Skip Filters")
  ...
  test("step 4: selecting a strategy and clicking Continue Setup advances to step 5", async ({
    page,
  }) => {
    const advanceTo = await mockUserWithAdvance(page, 4);
...
test.describe("Onboarding - Ultra Plan - Step 5 (Custom Instructions)", () => {
  ...
  test("step 5: clicking 'Start Email Generation' advances to step 6 (payment)", async ({
    ...
    await positionTextarea.fill(
      "I want to be a Principal Engineer at a top AI company"
    );
...
test.describe("Onboarding - Ultra Plan - Step 6 (Stripe Payment Form)", () => {
  test("user with onboardingStep=6 (ultra plan) sees payment form", async ({
    page,
  }) => {
  ...
  test("step 6: payment form shows Ultra Plan summary", async ({ page }) => {
    await mockStripeJs(page);
    await mockUserAtStep(page, 6);
    await page.goto("/dashboard");
  ...
  test("step 6: payment form shows Pay button with €139 amount", async ({
    page,
  }) => {
    await mockStripeJs(page);
    await mockUserAtStep(page, 6);
...
test.describe("Onboarding - Ultra Plan - Webhook & Dashboard Unlock", () => {
  test("after payment success, simulating webhook receipt unlocks main dashboard", async ({
    ...
    // (webhook: payment_intent.succeeded → updates plan=ultra, maxCompanies=100, credits=2500, onboardingStep=50)
  ...
  test("step 50: main dashboard shows correct Ultra Plan stats (maxCompanies=100, credits=2500)", async ({
    page,
  }) => {
    await mockUserAtStep(page, 50);
...
test.describe("Onboarding - Ultra Plan - Full End-to-End Flow", () => {
  test("full Ultra Plan flow: steps 1-5 -> payment form (step 6) -> mock success -> dashboard with maxCompanies=100 and credits=2500", async ({
    page,
  }) => {
    await mockStripeJs(page);
    await mockCreatePayment(page);
    ...
    const minimalPdf = Buffer.from(
      "%PDF-1.4\n1 0 obj<</Type/Catalog/Pages 2 0 R>>endobj\n" +
        "2 0 obj<</Type/Pages/Keys[3 0 R]/Count 1>>endobj\n" +
        "3 0 obj<</Type/Page/MediaBox[0 0 3 3]>>endobj\n" +
        "0000000114 00000 n\ntrailer<</Size 4/Root 1 0 R>>\n" +
        "startxref\n190\n%%EOF"
    );
    ...
    // Continue Setup (no "Skip Filters" button — only "Reset to Default" and "Continue Setup")
    const skipBtn = page.getByRole("button", { name: /continue setup/i });
    ...
    await expect(positionTextarea).toBeVisible({ timeout: 5000 });
    await positionTextarea.fill(
      "I want to be a Principal Engineer at the most innovative AI company"
    );
    ...
    // Webhook sets onboardingStep=50, plan=ultra, maxCompanies=100, credits=2500.
    advanceTo(50);
