...
function SearchExperience({ preview, replay = false, onReplayComplete }: { preview: OnboardingPreviewState; replay?: boolean; onReplayComplete?: () => void }) {
  ...
  return <div className="relative mx-auto flex min-h-[680px] max-w-5xl flex-col overflow-hidden">
...
function ActivationChapter({ step, label, title, story, signal, onBack, backPending, children }: { step: number; label: string; title: string; story: string; signal: string; onBack?: () => void; backPending?: boolean; children: React.ReactNode }) {
  const chapters = ['Your profile', 'Targets', 'Recruiter strategy', 'Message direction', 'Launch']
  return <div className="mx-auto max-w-5xl space-y-8">{onBack && <Button variant="ghost" size="sm" onClick={onBack} disabled={backPending} icon={backPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <ArrowLeft className="h-4 w-4" />}>Back</Button>}<div className="grid gap-7 lg:grid-cols-[1fr_300px] lg:items-end"><div><p className="text-xs uppercase tracking-[0.2em] text-violet-300">Chapter {step} · {label}</p><h1 className="mt-4 max-w-3xl text-4xl font-bold leading-tight text-white sm:text-5xl">{title}</h1><p className="mt-4 max-w-2xl text-lg leading-8 text-gray-400">{story}</p></div><Card hover={false} className="border-violet-400/20 bg-violet-500/[0.05] p-5"><Sparkles className="h-5 w-5 text-violet-300" /><p className="mt-3 text-xs uppercase tracking-[0.16em] text-gray-500">What this changes</p><p className="mt-2 text-sm leading-6 text-gray-200">{signal}</p></Card></div><div className="flex items-center gap-2">{chapters.map((chapter, index) => <div key={chapter} className="flex flex-1 items-center gap-2"><div className={`h-1.5 flex-1 rounded-full ${index + 1 <= step ? 'bg-violet-500' : 'bg-white/10'}`} /><span className={`hidden text-[10px] sm:block ${index + 1 === step ? 'text-violet-300' : 'text-gray-600'}`}>{chapter}</span></div>)}</div>{children}</div>
}
...
function PostPurchaseExperience({ props, preview }: { props: Props; preview: OnboardingPreviewState }) {
  ...
  if (props.stage === 'post_purchase_profile') return <ActivationChapter step={1} label="Strengthen the source" title="Before we search, make sure this is the story you want us to carry." story="Every recruiter strategy and every email begins here. Improve the profile now and CandidAI will rebuild its default search logic around the evidence you choose to keep." signal="Saving this profile recalculates the ordered recruiter strategies before you review them." onBack={() => backTo('post_purchase')} backPending={Boolean(pending)}><ProfileAnalysisClient userId={props.user.uid} plan={plan} initialProfile={props.profile} initialCvUrl={props.cvUrl} flow="guided" onSave={async (savedPlan, data, cv) => { await savePostPurchaseProfile(savedPlan, data, cv); router.refresh() }} /></ActivationChapter>
  if (props.stage === 'post_purchase_companies') return <ActivationChapter step={2} label="Set the map" title="Choose where this campaign should create openings." story="Each company becomes its own research track: a team to understand, a recruiter to earn attention from, and a message that cannot be reused anywhere else." signal={`Your ${planInfo?.name} plan can open up to ${maxCompanies} company tracks. Start with the ones you would genuinely accept a conversation from.`} onBack={() => backTo('post_purchase_profile')} backPending={Boolean(pending)}><CompanyInputClient userId={props.user.uid} maxCompanies={maxCompanies} initialCompanies={props.companies} planType={plan} isUltraPlan={plan === 'ultra'} mode="campaign-setup" onSave={savePostPurchaseCompanies} /></ActivationChapter>
  if (props.stage === 'post_purchase_filters') return <ActivationChapter step={3} label="Shape the search" title="Teach the search what a valuable introduction looks like." story={`Your improved profile has produced ${props.queries?.length || 0} ordered strategies. They move from precise common ground toward broader recruiter signals until the right person emerges.`} signal="Changing these filters changes who CandidAI considers worth contacting, not merely what appears in a form." onBack={() => backTo('post_purchase_companies')} backPending={Boolean(pending)}><AdvancedFiltersClientWrapper userId={props.user.uid} maxStrategies={plan === 'ultra' ? 50 : 30} defaultStrategy={props.queries || []} plan={plan} onSave={async queries => { await savePostPurchaseFilters(queries); router.refresh() }} /></ActivationChapter>
  if (props.stage === 'post_purchase_instructions') return <ActivationChapter step={4} label="Give it a voice" title="Give every message a reason to exist." story="The role defines the destination. Your instructions define the editorial judgment used to connect your strongest evidence with each company's reality." signal={plan === 'pro' || plan === 'ultra' ? 'These directions enter every generation, while company research keeps each final email distinct.' : 'Your target role becomes the common thread across every company-specific email.'} onBack={() => backTo(plan === 'pro' || plan === 'ultra' ? 'post_purchase_filters' : 'post_purchase_companies')} backPending={Boolean(pending)}><SetupCompleteClient userId={props.user.uid} defaultCustomizations={props.customizations} plan={plan} flow="activation" onSave={async data => { await savePostPurchaseInstructions(data); router.refresh() }} /></ActivationChapter>
  return <ActivationChapter step={5} label="Commit the campaign" title="Your campaign is assembled. See exactly where it is going." story="Your profile supplies the evidence. Every target below opens a separate research path, with its own recruiter selection and company-specific message." signal="Launching hands this complete campaign brief to the production pipeline. Nothing starts before you confirm." onBack={() => moveTo('post_purchase_instructions')} backPending={Boolean(pending)}>
    <Card hover={false} className="overflow-hidden border-blue-400/20 p-0"><div className="border-b border-white/10 bg-gradient-to-r from-blue-500/10 via-violet-500/5 to-transparent p-6 sm:p-8"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs uppercase tracking-[0.18em] text-blue-300">Campaign destinations</p><h2 className="mt-2 text-2xl font-semibold text-white">{props.companies?.length || 0} companies, {props.companies?.length || 0} original conversations.</h2></div><Button size="sm" variant="secondary" onClick={() => moveTo('post_purchase_companies', true)} icon={<Pencil className="h-4 w-4" />}>Edit targets</Button></div></div><div className="grid gap-3 p-5 sm:grid-cols-2 sm:p-6">{props.companies?.map((company, index) => <div key={`${company.name}-${index}`} className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/[0.025] p-4"><CompanyLogo company={company.domain || company.name} maxSize={11} minSize={11} /><div className="min-w-0 flex-1"><p className="truncate font-medium text-white">{company.name}</p><p className="mt-1 truncate text-xs text-gray-500">{company.domain || 'Company research prepared after launch'}</p></div><span className="h-2 w-2 shrink-0 rounded-full bg-emerald-400" /></div>)}</div></Card>
    <div className="grid gap-4 md:grid-cols-3"><button className="text-left" onClick={() => moveTo('post_purchase_profile', true)}><Card hover className="h-full p-6"><div className="flex items-center justify-between"><UserRound className="h-5 w-5 text-emerald-300" /><Pencil className="h-4 w-4 text-gray-600" /></div><p className="mt-4 font-semibold text-white">{props.profile?.name || 'Candidate profile'}</p><p className="mt-1 text-sm text-gray-400">{props.profile?.title || 'Your campaign evidence'}</p></Card></button><button className="text-left" onClick={() => moveTo(plan === 'pro' || plan === 'ultra' ? 'post_purchase_filters' : 'post_purchase_companies', true)}><Card hover className="h-full p-6"><div className="flex items-center justify-between"><SlidersHorizontal className="h-5 w-5 text-violet-300" /><Pencil className="h-4 w-4 text-gray-600" /></div><p className="mt-4 text-2xl font-bold text-white">{props.queries?.length || 'Default'}</p><p className="mt-1 text-sm text-gray-400">recruiter strategies configured</p></Card></button><button className="text-left" onClick={() => moveTo('post_purchase_instructions', true)}><Card hover className="h-full p-6"><div className="flex items-center justify-between"><Zap className="h-5 w-5 text-amber-300" /><Pencil className="h-4 w-4 text-gray-600" /></div><p className="mt-4 line-clamp-2 font-semibold text-white">{props.customizations?.position_description}</p><p className="mt-1 text-sm text-gray-400">campaign direction</p></Card></button></div>
