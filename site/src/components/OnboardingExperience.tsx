...
function PostPurchaseExperience({ props, preview }: { props: Props; preview: OnboardingPreviewState }) {
  ...
  return <ActivationChapter step={5} label="Commit the campaign" title="Your campaign is assembled. See exactly where it is going." story="Your profile supplies the evidence. Every target below opens a separate research path, with its own recruiter selection and company-specific message." signal="Launching hands this complete campaign brief to the production pipeline. Nothing starts before you confirm." onBack={() => moveTo('post_purchase_instructions')} backPending={Boolean(pending)}>
    <Card hover={false} className="overflow-hidden border-blue-400/20 p-0"><div className="border-b border-white/10 bg-gradient-to-r from-blue-500/10 via-violet-500/5 to-transparent p-6 sm:p-8"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs uppercase tracking-[0.18em] text-blue-300">Campaign destinations</p><h2 className="mt-2 text-2xl font-semibold text-white">{props.companies?.length || 0} companies, {props.companies?.length || 0} original conversations.</h2></div><Button size="sm" variant="secondary" onClick={() => moveTo('post_purchase_companies', true)} icon={<Pencil className="h-4 w-4" />}>Edit targets</Button></div></div><div className="grid gap-3 p-5 sm:grid-cols-2 sm:p-6">{props.companies?.map((company, index) => <div key={`${company.name}-${index}`} className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/[0.025] p-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500/20 to-violet-500/20 font-semibold text-blue-200">{company.name.slice(0, 2).toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate font-medium text-white">{company.name}</p><p className="mt-1 truncate text-xs text-gray-500">{company.domain || 'Company research prepared after launch'}</p></div><span className="h-2 w-2 shrink-0 rounded-full bg-emerald-400" /></div>)}</div></Card>
