...
function PostPurchaseExperience({ props, preview }: { props: Props; preview: OnboardingPreviewState }) {
  ...
  if (props.stage === 'post_purchase_companies') return <ActivationChapter step={1} label="Set the map" title="Choose where this campaign should create openings." story="Each company becomes its own research track: a team to understand, a recruiter to earn attention from, and a message that cannot be reused anywhere else." signal={`Your ${planInfo?.name} plan can open up to ${maxCompanies} company tracks. Start with the ones you would genuinely accept a conversation from.`}><CompanyInputClient userId={props.user.uid} maxCompanies={maxCompanies} initialCompanies={props.companies} planType={plan} isUltraPlan={plan === 'ultra'} mode="campaign-setup" onSave={savePostPurchaseCompanies} /></ActivationChapter>
  if (props.stage === 'post_purchase_filters') return <ActivationChapter step={2} label="Shape the search" title="Teach the search what a valuable introduction looks like." story={`Your profile has already produced ${props.queries?.length || 0} ordered strategies. They move from precise common ground toward broader recruiter signals until the right person emerges.`} signal="Changing these filters changes who CandidAI considers worth contacting, not merely what appears in a form."><AdvancedFiltersClientWrapper userId={props.user.uid} maxStrategies={plan === 'ultra' ? 50 : 30} defaultStrategy={props.queries || []} plan={plan} onSave={async queries => { await savePostPurchaseFilters(queries); router.refresh() }} /></ActivationChapter>
  if (props.stage === 'post_purchase_instructions') return <ActivationChapter step={3} label="Give it a voice" title="Give every message a reason to exist." story="The role defines the destination. Your instructions define the editorial judgment used to connect your strongest evidence with each company's reality." signal={plan === 'pro' || plan === 'ultra' ? 'These directions enter every generation, while company research keeps each final email distinct.' : 'Your target role becomes the common thread across every company-specific email.'}><SetupCompleteClient userId={props.user.uid} defaultCustomizations={props.customizations} plan={plan} flow="activation" onSave={async data => { await savePostPurchaseInstructions(data); router.refresh() }} /></ActivationChapter>
  return <ActivationChapter step={4} label="Commit the campaign" title="The pieces now describe one coherent search." story="Targets define where to look. Your strategy defines who matters. Your direction defines what each conversation should make possible." signal="Launching hands this complete campaign brief to the production pipeline. Nothing starts before you confirm."><div className="grid gap-4 md:grid-cols-3"><Card hover={false} className="p-6"><Building2 className="h-5 w-5 text-blue-300" /><p className="mt-4 text-2xl font-bold text-white">{props.companies?.length || 0}</p><p className="mt-1 text-sm text-gray-400">target companies</p></Card><Card hover={false} className="p-6"><SlidersHorizontal className="h-5 w-5 text-violet-300" /><p className="mt-4 text-2xl font-bold text-white">{props.queries?.length || 'Default'}</p><p className="mt-1 text-sm text-gray-400">recruiter strategies configured</p></Card><Card hover={false} className="p-6"><Zap className="h-5 w-5 text-amber-300" /><p className="mt-4 text-lg font-semibold text-white">{props.customizations?.position_description}</p><p className="mt-1 text-sm text-gray-400">campaign direction</p></Card></div><Card hover={false} className="border-emerald-400/20 p-6 text-center sm:p-8"><h2 className="text-2xl font-semibold text-white">Everything is saved. Ready when you are.</h2><p className="mx-auto mt-3 max-w-2xl text-sm leading-7 text-gray-400">The free preview remains available as Version 1 only if you kept its company. The definitive pipeline will research every target and select every recruiter again from scratch.</p><Button size="lg" className="mt-6" disabled={Boolean(pending)} onClick={() => run('launch', launchPostPurchaseCampaign)} icon={pending ? <Loader2 className="h-4 w-4 animate-spin" /> : <ArrowRight className="h-4 w-4" />}>{pending ? 'Launching...' : 'Launch my campaign'}</Button>{error && <p className="mt-4 text-sm text-red-300">{error}</p>}</Card></ActivationChapter>
}
