        ...
        <>
            {experience?.length ? (
                experience.map((exp, idx) => (
                    <Link key={idx} href={exp.company.linkedin_url ? ("https://" + exp.company.linkedin_url) : "#"} target="_blank" rel="noopener noreferrer">
                        <li
                            key={idx}
...
export const calculateProgress = (data: any) => {
    if (data.email || data.email_sent !== undefined) return 100;
    let progress = 0;
