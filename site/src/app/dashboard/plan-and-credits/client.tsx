    ...
    return (
        <motion.div className="space-y-14" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65 }}>
            ...
            <section className="space-y-7 text-center">
                ...
                <div className="flex snap-x snap-mandatory gap-3 overflow-x-auto pb-3 md:grid md:grid-cols-[1fr_auto_1fr_auto_1fr] md:items-stretch md:overflow-visible md:pb-0">
                    {[{ icon: Building2, number: '01', title: 'Understand the company', text: 'Build the context needed to approach the right team with a relevant angle.' }, { icon: Search, number: '02', title: 'Find the strongest recruiter', text: 'Test increasingly broad strategies until a credible match emerges.' }, { icon: Mail, number: '03', title: 'Write for that connection', text: 'Generate a personal email grounded in your profile, the company, and the recruiter.' }].map((item, index) => <div className="w-[80%] shrink-0 snap-center first:ml-[6%] last:mr-[6%] md:contents" key={item.title}><Card hover={false} className="p-6 text-left"><div className="flex items-center justify-between"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-violet-500/15 text-violet-300"><item.icon className="h-5 w-5" /></div><span className="text-xs text-gray-600">{item.number}</span></div><h3 className="mt-5 text-lg font-semibold text-white">{item.title}</h3><p className="mt-2 text-sm leading-6 text-gray-400">{item.text}</p></Card>{index < 2 && <ArrowRight className="mx-auto hidden h-5 w-5 self-center text-violet-500/50 md:block" />}</div>)}
                </div>
            </section>
